...
class BinaryTree:
    ...
    def get_parent_pointer(self, node):
        return node.parent
...
def main():
    list_binary_tree = [(2,1,5),(5,3,6), (3,'',4),]
    bin_tree = BinaryTree()
    ...
    for nodes in list_binary_tree:
        ...
        if parent:
            ...
            if left_node:
                ...
                print left_node.parent.data, parent_data

            if right_node:
                print "Inserting right node: ", nodes[2]
                parent.right = right_node
                right_node.parent = parent
                print right_node.parent.data, parent_data
    ...
    for i in range(1,7,1):
        node = bin_tree.get_parent(bin_tree.root,i, None)
        print node, i
        print "Finding inorder successor for ", node.data
        successor = bin_tree.get_inorder_successor(node)
        if successor:
